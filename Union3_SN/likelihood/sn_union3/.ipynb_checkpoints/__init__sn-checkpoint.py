...
class sn_union3(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        self.names = []
        ix = 0
        with open(os.path.join(self.data_directory, self.data_file), 'r') as f:
            lines = f.readlines()
            for line in lines:
                if '#' in line:
                    cols = line[1:].split()
                    ...
                    if cols is None:
                        raise LoggedError('Data file must have comment header')
                    vals = line.split()
