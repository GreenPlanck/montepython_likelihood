...
class desi_fs_bao_all(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        print("[desi_fs_bao_all] Fitting {}.".format(self.zbins))
        ...
        self.theory = reptvelocileptors()
        class_args = self.theory.must_provide(
            **self._requirements
        )
        print(class_args)
    ...
    def get_bao_flattheory(self, z, params):
        ...
        rdrag = float(self.cosmo.rs_drag())
        apar = np.squeeze(
            self.fiducial.efunc(z)
            / (np.asarray(self.cosmo.Hubble(z)) / float(self.cosmo.Hubble(0.0)))
            * self.fiducial.rs_drag
            / (float(self.cosmo.h()) * rdrag)
        )
        aper = np.squeeze(
            np.asarray(self.cosmo.angular_distance(z))
            / rdrag
            / (self.fiducial.angular_diameter_distance(z) / self.fiducial.rs_drag)
        )
    ...
    #def loglkl(self, _derived=None, **params_values):
    def loglkl(self, cosmo, data):
        ...
        for i, (tracer, zrange, name) in enumerate(self.zbins):
            ...
            if not has_no_fs:
                ...
                fs_theory, fs_gradient = self.theory.get_pkpoles(fs_params, z=z, **settings, sn=self.shotnoise[i], return_gradient=True)
