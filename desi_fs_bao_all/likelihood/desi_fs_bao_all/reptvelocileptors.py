...
class reptvelocileptors:
    ...
    def must_provide(self, **requirements):
        """Configure REPT and return the required CLASS arguments."""

        config = requirements["pkpoles"]

        self.z = np.asarray(config["z"])
        self.k = np.asarray(config["k"])
        self.ells = tuple(config["ells"])
        ...
        return {
            "output": "mPk",
            "P_k_max_1/Mpc": 2.0,
            "z_max_pk": float(np.max(self.z)),
        }

    # def set_template(self,cosmo):
    ...
    def set_template(self, cosmo):
        """Set the linear density, velocity-divergence, and no-wiggle spectra."""
        h = float(cosmo.h())
        k = np.asarray(self.kin)

        pk_dd = np.empty((k.size, len(self.z)), dtype="f8")
        fk = np.empty_like(pk_dd)

        for iz, z in enumerate(self.z):
            for ik, kh in enumerate(k):
                pk_dd[ik, iz] = cosmo.pk_cb_lin(kh * h, float(z)) * h**3
                fk[ik, iz] = cosmo.scale_dependent_growth_factor_f_cb(
                    kh, float(z), h_units=True, nonlinear=False
                )

        if not np.all(np.isfinite(pk_dd)) or np.any(pk_dd <= 0.):
            raise ValueError("CLASS returned a non-positive or non-finite P_cb")
        if not np.all(np.isfinite(fk)) or np.any(fk <= 0.):
            raise ValueError("CLASS returned a non-positive or non-finite f_cb")

        pk_tt = fk**2 * pk_dd

        def pk_grid(*var_pair):
            """Return k in h/Mpc and P(k,z) in (Mpc/h)^3."""
            if var_pair == ("delta_nonu", "delta_nonu"):
                return k, pk_dd

            velocity_pairs = {
                ("v_newtonian_baryon", "v_newtonian_baryon"),
                ("v_newtonian_baryon", "v_newtonian_cdm"),
                ("v_newtonian_cdm", "v_newtonian_baryon"),
                ("v_newtonian_cdm", "v_newtonian_cdm"),
            }
            if var_pair in velocity_pairs:
                return k, pk_tt
            raise ValueError("Unsupported power-spectrum pair: {}".format(var_pair))

        k, pk = pk_grid("delta_nonu", "delta_nonu")
        pk_dd_interpolator = PowerSpectrumInterpolator1D(k, pk)

        Omega_b = float(cosmo.Omega_b())
        Omega_cdm = float(cosmo.Omega0_cdm())
        Omega_ncdm = float(cosmo.Omega_nu())
        ...
        filter_cosmo = Cosmology(**cstate)
        filter_cosmo.rs_drag = float(cosmo.rs_drag()) * h

        self._template["filter"](pk_dd_interpolator, cosmo=filter_cosmo)
        pknow_dd_interpolator = self._template["filter"].smooth_pk_interpolator()

        fb = Omega_b / (Omega_cdm + Omega_b)
        pk = (
            fb**2 * pk_grid("v_newtonian_baryon", "v_newtonian_baryon")[-1]
            + 2. * fb * (1. - fb) * pk_grid("v_newtonian_baryon", "v_newtonian_cdm")[-1]
            + (1. - fb)**2 * pk_grid("v_newtonian_cdm", "v_newtonian_cdm")[-1]
        )
        pk_tt_interpolator = PowerSpectrumInterpolator1D(k, pk)
    ...
    def calculate(self, state, cosmo=None, want_derived=True, **params_values_dict):
        """Set theory power spectrum multipoles."""
        self.set_template(cosmo)
        ...
        H0_class = float(cosmo.Hubble(0.0))
        E_class = np.array([float(cosmo.Hubble(float(z))) / H0_class for z in self.z])
        qpar = (self.fiducial.efunc(self.z)/ E_class)
        DA_class = np.array([float(cosmo.angular_distance(float(z))) for z in self.z])
        qper = (DA_class* h / self.fiducial.angular_diameter_distance(self.z))
        ...
        self.current_state = {
        "pkpoles": pktable,
        "sigma8": np.asarray(sigma8),
        "fsigma8": np.asarray(fsigma8)}
